"""
Скрипт импорта каталога Эксмо из JSON в таблицу Book (source=eksmo).

Использование:
    python import_eksmo_books.py
    python import_eksmo_books.py --update
    python import_eksmo_books.py --dry-run
    python import_eksmo_books.py --file /путь/к/файлу.json
"""
...
DEFAULT_JSON = Path(__file__).resolve().parent / 'parsing' / 'eksmo_books_mt_deduped.json'
...
def import_eksmo_books(file_path: str, update: bool = False, dry_run: bool = False):
    ...
    if not isinstance(rows, list):
        print('❌ JSON должен быть массивом объектов', file=sys.stderr)
        return
    ...
    print(f'Записей в файле: {len(rows)}, к импорту: {len(books)}, пропущено: {skipped}')

    if dry_run:
        print('⚠️ Dry-run: в БД ничего не записано')
        return
...
def main():
    parser = argparse.ArgumentParser(description='Импорт каталога Эксмо из JSON в БД')
