...
Основная область применения — управление букинистическими и обычными книгами: импорт каталогов из JSON, ручное редактирование
товаров, загрузка фото, экспорт выбранных книг в Excel и в Excel-шаблон Ozon
...
- поиск книги Эксмо по названию или ISBN
- автоматическая подстановка данных книги Эксмо в форму ручной книги

### Поиск книги Эксмо

При создании ручной книги в форме появляется блок «Поиск в каталоге Эксмо».
Логика:

1. Пользователь вводит название или ISBN.
2. AJAX-запрос обращается в `search-eksmo/`.
3. Для ISBN-запросов нормализуются цифры и дефисы.
4. Найденная книга Эксмо подставляется в форму.
5. При сохранении создаётся новая запись с `source='manual'`.
