...
### Экспорт в Avito XML — ⚠️ НЕ РАБОТАЕТ

Действие в админке: **«Экспортировать выбранные в Avito XML»** (`export_to_avito_xml` в `admin_panel/admin.py`, функция `export_books_to_avito_xml` в `admin_panel/views.py`).

Формирует XML по схеме Avito:

- Корневой тег `<Ads formatVersion="3" target="Avito.ru">`
- Кодировка UTF-8
- Обязательные поля: `Id`, `Title`, `Description`, `Price`, `Category`, `GoodsType`, `AdType`, `Condition`, `BookType`, `Genre`, `Address`
- `Author` / `Series` добавляются только если значение есть в каталоге Авито (`admin_panel/avito_catalogs.py`: 4723 автора, 8 серий)

#### ⚠️ Важное предупреждение

**Экспорт Avito XML на текущий момент НЕ работает.** Автозагрузка Авито отклоняет объявления, жалуясь на параметры **`Author` / Автор** и **`Series` / Популярная серия** (ошибка «Значение не найдено»). Значения из каталога книг не совпадают со справочником Авито, а полный маппинг на справочник пока не реализован. Экспорт можно использовать только для тестирования структуры XML, но не для публикации в Авито.
...
14. **Экспорт Avito XML не работает.**  
    Действие «Экспортировать выбранные в Avito XML» формирует файл, но Авито отклоняет объявления из-за параметров **`Author` / Автор** и **`Series` / Популярная серия** (ошибка «Значение не найдено»). Подробнее — раздел **8. Экспорт → Экспорт в Avito XML**.
